...
st.set_page_config(page_title="CRM Intelligence Dashboard v10", layout="wide")
...
def show_health_distribution(buyer_health, title="Buyer Health Distribution"):
    ...
    st.caption("Final Health Status คำนวณจาก Health Score: HOT >=65, WARM >=35 and <65, COOL >0 and <35, COLD =0. Rolling 3M: Login % = Total Login 3M / 30 และ Buy Units/Month = Total Units 3M / 3")
    return dist
...
st.title("CRM Intelligence Dashboard v10")
...
current_kpi_label = selected_month if view_mode == "Snapshot" else f"First month to {selected_month}"
...
st.caption(f"Buyer Snapshot: {selected_month} | KPI Mode: {view_mode} ({current_kpi_label}) | Rolling Health Months: {', '.join(rolling_months)}")
...
if page.startswith("1 "):
    ...
    st.caption("หมายเหตุ: Health Distribution ใช้ Rolling 3M ตาม Snapshot Month ที่เลือก ไม่ได้เปลี่ยนตาม KPI Mode เพราะไม่ใช่ snapshot/accumulated KPI")
...
Final Health Status: HOT >=65, WARM >=35 and <65, COOL >0 and <35, COLD =0""")
    ...
    st.plotly_chart(px.bar(login_dist, x="Buyers", y="Login_Status", orientation="h", color="Login_Status", text="Buyers", color_discrete_map=STATUS_COLORS, category_orders={"Login_Status": LOGIN_ORDER}), use_container_width=True)

    st.markdown("#### Buy Status Distribution")
    buy_dist = status_summary(buyer_health, "Buy_Status", BUY_ORDER)
    st.plotly_chart(px.bar(buy_dist, x="Buyers", y="Buy_Status", orientation="h", color="Buy_Status", text="Buyers", color_discrete_map=STATUS_COLORS, category_orders={"Buy_Status": BUY_ORDER}), use_container_width=True)
