...
st.set_page_config(page_title="CRM Intelligence Dashboard v11", layout="wide")
...
def style_fig(fig, height=420, legend_top=True, showlegend=None):
    """Consistent responsive Plotly layout. Keeps legends inside visible area."""
    fig.update_layout(
        height=height,
        margin=dict(l=40, r=40, t=80 if legend_top else 50, b=60),
        font=dict(size=12),
...
def plot(fig, height=420, legend_top=True, showlegend=None):
    st.plotly_chart(style_fig(fig, height=height, legend_top=legend_top, showlegend=showlegend), use_container_width=True)
...
def final_health_status(score):
    # Final Health Status follows the same business bands as the locked score logic.
    # This prevents Login-HOT buyers with low purchase score from being incorrectly hidden.
    if score >= 65:
        return "HOT"
    if score >= 35:
        return "WARM"
...
def build_rolling_health(history_df, snapshot_month, customer_col, province_col, ae_col, login_col, bid_col, buy_count_col, unit_col):
    ...
    out["Buy_Units_Per_Month"] = out["Total_Buy_Units_3M"] / 3
...
def show_status_distribution(df, status_col, order, title, caption=""):
    dist = status_summary(df, status_col, order)
    st.markdown(f"#### {title}")
    cols = st.columns(len(order))
    for i, status in enumerate(order):
        row = dist[dist[status_col] == status].iloc[0]
        cols[i].metric(f"{status} Buyers", f"{int(row['Buyers']):,}", f"{row['%']:.1f}%")
    fig = px.bar(
        dist, y=status_col, x="Buyers", orientation="h",
        text=dist.apply(lambda r: f"{int(r['Buyers']):,} ({r['%']:.1f}%)", axis=1),
        color=status_col, color_discrete_map=STATUS_COLORS, category_orders={status_col: order}
    )
    fig.update_yaxes(categoryorder="array", categoryarray=list(reversed(order)))
    fig.update_traces(textposition="outside", cliponaxis=False)
    fig.update_layout(yaxis_title=None, xaxis_title="Buyers")
    plot(fig, height=320, legend_top=False, showlegend=False)
    if caption:
        st.caption(caption)
    return dist


def show_health_distribution(buyer_health, title="Final Health Score Distribution"):
    return show_status_distribution(
        buyer_health, "Health_Status", HEALTH_ORDER, title,
        "Final Health Status = Health Score bands. ถ้า HOT เป็น 0 ให้ดู Login Status แยกด้วย เพราะผู้ที่ Login HOT อาจยังไม่เป็น Final HOT หาก Buy Score ต่ำ."
    )
...
st.title("CRM Intelligence Dashboard v11")
...
selected_month = st.sidebar.selectbox("Selected Month", months, index=len(months)-1, help="เดือนปลายทางของการแสดงผล: Current Snapshot ใช้เดือนนี้เท่านั้น, Accumulated รวมถึงเดือนนี้, Rolling 3M ใช้ 3 เดือนล่าสุดนับถึงเดือนนี้")
data_scope = st.sidebar.radio("Data Scope", ["Current Snapshot", "Accumulated to Selected Month"], index=0, help="Current Snapshot = เดือนเดียว, Accumulated to Selected Month = รวมตั้งแต่เดือนแรกจนถึงเดือนที่เลือก")
view_mode = "Snapshot" if data_scope == "Current Snapshot" else "Accumulated"
...
current_kpi_df = snapshot_df.copy() if view_mode == "Snapshot" else accum_df.copy()
first_month = months[0]
current_kpi_label = selected_month if view_mode == "Snapshot" else f"{first_month} → {selected_month}"
...
active_scope_text = "Current Snapshot" if view_mode == "Snapshot" else "Accumulated to Selected Month"

# Explicit KPI totals by layer. Every KPI card and mode-sensitive chart reads from this object,
# so Snapshot and Accumulated cannot silently point to the same dataframe.
def kpi_totals(df):
    return {
        "Buyers": int(df[customer_col].nunique()),
        "Login": int(to_num(df[login_col]).sum()),
        "Bid": int(to_num(df[bid_col]).sum()),
        "Buy": int(to_num(df[buy_count_col]).sum()),
        "Units": int(to_num(df[unit_col]).sum()),
    }

snapshot_kpi = kpi_totals(snapshot_df)
accumulated_kpi = kpi_totals(accum_df)
selected_kpi = snapshot_kpi if view_mode == "Snapshot" else accumulated_kpi
...
st.caption(f"Data Scope: {active_scope_text} | Selected Month: {selected_month} | KPI Window: {current_kpi_label} | Rolling Health Months: {', '.join(rolling_months)}")
...
if page.startswith("1 "):
    ...
    c1.metric("Buyers", f"{selected_kpi["Buyers"]:,}")
    c2.metric("Login", f"{selected_kpi["Login"]:,}")
    c3.metric("Bid", f"{selected_kpi["Bid"]:,}")
    c4.metric("Buy", f"{selected_kpi["Buy"]:,}")
    c5.metric("Visits", f"{len(visit_filtered):,.0f}")

    with st.expander("KPI Layer Validation", expanded=False):
        st.write("Snapshot ใช้เฉพาะเดือนที่เลือก ส่วน Accumulated รวมตั้งแต่เดือนแรกถึงเดือนที่เลือก")
        st.dataframe(pd.DataFrame([
            {"Mode": "Snapshot", **snapshot_kpi},
            {"Mode": "Accumulated", **accumulated_kpi},
        ]), use_container_width=True)

    section_title("Engagement & Final Health Distribution", "แยก Login Status ออกจาก Final Health Score เพื่อลดความสับสนเรื่อง HOT")
    show_status_distribution(
        buyer_health, "Login_Status", LOGIN_ORDER, "Login Status Distribution",
        "Login Status ใช้ Rolling 3M: Login % = Total Login ล่าสุด 3 เดือน / 30. หมวด HOT/WARM/COOL/COLD นี้คือ Engagement เท่านั้น."
    )
    show_health_distribution(buyer_health, "Final Health Score Distribution")

    section_title("Historical CRM Trend", "เลือกดูทีละ metric เพื่อไม่ให้ legend ตกขอบหรือกราฟรก")
    ...
    if chart_or_note(trend):
        metric_choice = st.selectbox("Trend Metric", ["Login", "Bid", "Buy", "Units", "Buyers"], index=0)
        fig = px.line(trend, x="Snapshot_Month", y=metric_choice, markers=True, text=metric_choice)
        fig.update_traces(textposition="top center")
        fig.update_layout(xaxis_title="Snapshot Month", yaxis_title=metric_choice)
        plot(fig, height=420, legend_top=False, showlegend=False)

    section_title("Top Provinces by Buy", "จังหวัดที่สร้าง Buy สูงสุดตาม Data Scope ที่เลือก")
    top_province = province_summary.sort_values("Total_Buy", ascending=False).head(15)
    if chart_or_note(top_province):
        fig = px.bar(top_province, x="Total_Buy", y="Province", orientation="h", text="Total_Buy")
        plot(fig, height=420, legend_top=False, showlegend=False)
    ...
    section_title("Buyers by Province", "ฐานลูกค้าตามจังหวัดจาก Data Scope ที่เลือก")
    if chart_or_note(province_summary):
        fig = px.bar(province_summary.sort_values("Buyers", ascending=False).head(25), x="Province", y="Buyers", text="Buyers")
        plot(fig, height=460, legend_top=False, showlegend=False)
    ...
    if chart_or_note(province_opp):
        fig = px.scatter(province_opp, x="Coverage_Rate", y="Total_Units", size="Buyers", color="Opportunity_Level", hover_name="Province", color_discrete_map=STATUS_COLORS, category_orders={"Opportunity_Level": OPP_ORDER})
        plot(fig, height=500, legend_top=True)
    ...
    if chart_or_note(hp):
        fig = px.bar(hp, x="Province", y="Count", color="Health_Status", color_discrete_map=STATUS_COLORS, category_orders={"Health_Status": HEALTH_ORDER})
        fig.update_layout(barmode="stack", xaxis_tickangle=-45)
        plot(fig, height=560, legend_top=True)
    ...
    fig = px.bar(ae_perf.melt(id_vars="AE", value_vars=["Login", "Bid", "Buy"], var_name="Stage", value_name="Value"), x="AE", y="Value", color="Stage", barmode="group")
    plot(fig, height=420, legend_top=True)
    ...
    fig = px.bar(ah, x="AE", y="Count", color="Health_Status", color_discrete_map=STATUS_COLORS, category_orders={"Health_Status": HEALTH_ORDER})
    fig.update_layout(barmode="stack")
    plot(fig, height=430, legend_top=True)
...
Health Score = (Login Score x 50%) + (Buy Score x 50%)
Final Health Status: HOT >=65, WARM >=35 and <65, COOL >0 and <35, COLD =0""")
    show_health_distribution(buyer_health, "Final Health Distribution")

    st.markdown("#### Login Status Distribution")
    login_dist = status_summary(buyer_health, "Login_Status", LOGIN_ORDER)
    fig = px.bar(login_dist, x="Buyers", y="Login_Status", orientation="h", color="Login_Status", text="Buyers", color_discrete_map=STATUS_COLORS, category_orders={"Login_Status": LOGIN_ORDER})
    plot(fig, height=320, legend_top=False, showlegend=False)

    st.markdown("#### Buy Status Distribution")
    buy_dist = status_summary(buyer_health, "Buy_Status", BUY_ORDER)
    fig = px.bar(buy_dist, x="Buyers", y="Buy_Status", orientation="h", color="Buy_Status", text="Buyers", color_discrete_map=STATUS_COLORS, category_orders={"Buy_Status": BUY_ORDER})
    plot(fig, height=320, legend_top=False, showlegend=False)
    ...
    fig = px.density_heatmap(matrix, x="Login_Status", y="Buy_Status", z="Buyers", text_auto=True, category_orders={"Login_Status": LOGIN_ORDER, "Buy_Status": BUY_ORDER})
    plot(fig, height=420, legend_top=True)
    ...
    fig = px.scatter(province_opp, x="Coverage_Rate", y="Total_Bid", size="Buyers", color="Opportunity_Level", hover_name="Province", color_discrete_map=STATUS_COLORS, category_orders={"Opportunity_Level": OPP_ORDER})
    plot(fig, height=500, legend_top=True)
    ...
    fig = px.bar(dist, x="Provinces", y="Opportunity_Level", orientation="h", color="Opportunity_Level", color_discrete_map=STATUS_COLORS, category_orders={"Opportunity_Level": OPP_ORDER}, text="Provinces")
    plot(fig, height=330, legend_top=False, showlegend=False)
    ...
    if visit_date_col and visit_filtered[visit_date_col].notna().any():
        ...
        fig = px.line(vt.groupby("Month").size().reset_index(name="Visits"), x="Month", y="Visits", markers=True, text="Visits")
        fig.update_traces(textposition="top center")
        plot(fig, height=420, legend_top=False, showlegend=False)
    else:
    ...
    if visit_purpose_col:
        ...
        fig = px.bar(purpose.head(20), x="Count", y=visit_purpose_col, orientation="h")
        plot(fig, height=460, legend_top=False, showlegend=False)
    else:
    ...
    if demand_filtered.empty:
        ...
        if segment_col:
            ...
            fig = px.bar(seg, x="Demand", y=segment_col, orientation="h")
            plot(fig, height=460, legend_top=False, showlegend=False)
        ...
        if brand_col:
            ...
            fig = px.bar(brand, x="Demand", y=brand_col, orientation="h")
            plot(fig, height=460, legend_top=False, showlegend=False)
        ...
        if model_col:
            ...
            fig = px.bar(model, x="Demand", y=model_col, orientation="h")
            plot(fig, height=460, legend_top=False, showlegend=False)
        ...
        if year_col:
            ...
            fig = px.bar(year, x=year_col, y="Demand")
            plot(fig, height=420, legend_top=False, showlegend=False)
    ...
    if feedback_filtered.empty:
        ...
        if fb_category_col:
            ...
            fig = px.bar(fbc, x="Count", y=fb_category_col, orientation="h")
            plot(fig, height=460, legend_top=False, showlegend=False)
        ...
        if fb_sentiment_col:
            ...
            fig = px.pie(fs, names=fb_sentiment_col, values="Count")
            plot(fig, height=420, legend_top=True)
    ...
    if not follow_required_col:
        ...
        fig = px.bar(fu, x="Follow_Up_Flag", y="Count", text="Count")
        plot(fig, height=360, legend_top=False, showlegend=False)
        if visit_ae_col:
            ...
            fig = px.bar(fuae, x=visit_ae_col, y="Count", color="Follow_Up_Flag", barmode="group")
            plot(fig, height=420, legend_top=True)
