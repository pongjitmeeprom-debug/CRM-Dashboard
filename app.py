...
def add_status_legend():
    ...
    <span style='background:#00C853;color:white;padding:4px 10px;border-radius:12px'>HOT/HIGH</span>
    <span style='background:#FFB300;color:white;padding:4px 10px;border-radius:12px'>WARM/MEDIUM</span>
    <span style='background:#42A5F5;color:white;padding:4px 10px;border-radius:12px'>COOL/LOW</span>
    <span style='background:#EF5350;color:white;padding:4px 10px;border-radius:12px'>COLD/VERY LOW</span>
...
follow_col = find_col(visit_df, ["Feedback_Action_Needed", "Need_Follow_Up", "Follow_Up_Status", "Follow-up Status", "Followup_Status"])
...
buyer_health["Buy_Avg_Month"] = buyer_health["Total_Buy"] / period_count
...
if visit_province_col and not visit_filtered.empty:
    visit_summary = visit_filtered.groupby(visit_province_col)[visit_customer_col].nunique().reset_index(name="Visits").rename(columns={visit_province_col: "Province"})
else:
...
if visit_customer_col and visit_customer_col in visit_filtered.columns:
    buyer_visit_count = visit_filtered.groupby(visit_customer_col)[visit_customer_col].nunique().reset_index(name="Visits").rename(columns={visit_customer_col: customer_col})
...
buyer_health["Churn_Risk"] = buyer_health.apply(lambda r: "High Churn" if r["Health_Status"] == "COLD" and r["Total_Buy"] == 0 else ("At Risk" if r["Health_Status"] in ["COOL","COLD"] else "Healthy"), axis=1)
...
if page.startswith("1 "):
    ...
    section_title("Buyer Health Distribution", "ดูสัดส่วนลูกค้า HOT / WARM / COOL / COLD จากนิยาม Login และ Buy ใหม่")
    heat = buyer_health.groupby("Health_Status").size().reset_index(name="Count")
    if chart_or_note(heat):
        st.plotly_chart(px.pie(heat, names="Health_Status", values="Count", color="Health_Status", color_discrete_map=STATUS_COLORS), use_container_width=True)
    ...
    section_title("Province Opportunity Matrix", "เทียบ Visit, Buy, Buyer Size และ Opportunity Score เพื่อหาพื้นที่ที่ควร prioritize")
    st.plotly_chart(px.scatter(province_opp, x="Visits", y="Total_Buy", size="Buyers", color="Opportunity_Score", hover_name="Province"), use_container_width=True)
...
Coverage Gap source: Visit Count by Province""")
    ...
    st.plotly_chart(px.scatter(province_opp, x="Visits", y="Total_Bid", size="Buyers", color="Opportunity_Level", hover_name="Province", color_discrete_map=STATUS_COLORS), use_container_width=True)
